...
class EarthAttention3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        earth_position_bias = self.earth_position_bias_table[self.earth_position_index.clone().view(-1)].view(
...
class EarthSpecificBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=None, shift_size=None, mlp_ratio=4.,
        ...
        super().__init__()
        window_size = (2, 6, 6) if window_size is None else window_size
        shift_size = (1, 3, 3) if shift_size is None else shift_size
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
...
class Pangu_lite(nn.Module):
    ...
    Args:
        ...
        window_size (tuple[int]): Window size.
    """
    def __init__(self, patch_size=(2,4,4), num_air_variables=5, num_surface_variables=5, embed_dim=96, 
                 type_of_windows=(124,64,64,124), depths=(2,6,6,2), num_heads=(2, 4, 4, 2), window_size=(2, 6, 6), drop_rate=0.2):
        super().__init__()
        drop_path = np.linspace(0, 0.2, 8).tolist()
        # In addition, three constant masks(the topography mask, land-sea mask and soil type mask)
        self.patchembed2d = PatchEmbed2D(
        ...
        self.layer1 = BasicLayer(
            dim=embed_dim,
            input_resolution=(8, 61, 61),
            depth=2,
            num_heads=num_heads[0],
            window_size=window_size,
            drop_path=drop_path[:2]
        )
        self.downsample = DownSample(in_dim=embed_dim, input_resolution=(8, 61, 61), output_resolution=(8, 31, 31))
        self.layer2 = BasicLayer(
            dim=embed_dim * 2,
            input_resolution=(8, 31, 31),
            depth=6,
            num_heads=num_heads[1],
            window_size=window_size,
            drop_path=drop_path[2:]
        ...
        self.upsample = UpSample(embed_dim * 2, embed_dim, (8, 31, 31), (8, 61, 61))
        self.layer4 = BasicLayer(
            dim=embed_dim,
            input_resolution=(8, 61, 61),
            depth=2,
        ...
        self.patchrecovery2d = PatchRecovery2D((241, 241), (4, 4), 2 * embed_dim, 5)
        self.patchrecovery3d = PatchRecovery3D((13, 241, 241), (2, 4, 4), 2 * embed_dim, 5)
    ...
    def forward(self, surface, surface_mask, upper_air):
        ...
        Args:
            ...
            upper_air (torch.Tensor): 3D n_pl=13, n_lat=721, n_lon=1440, chans=5.
        """
        surface = torch.concat([surface, surface_mask.unsqueeze(0).repeat(surface.shape[0],1,1,1)], dim=1) # b,variables,lat,lon
        surface = self.patchembed2d(surface) # b, embed_dim, patch_lat, patch_lon
        upper_air = self.patchembed3d(upper_air)  # b, embed_dim, patch_levles, patch_lat, patch_lon

        x = torch.concat([surface.unsqueeze(2), upper_air], dim=2)
        B, C, Pl, Lat, Lon = x.shape
        x = x.reshape(B, C, -1).transpose(1, 2) # b, -1, embeded_dim
