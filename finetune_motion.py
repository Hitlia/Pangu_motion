...
def load_model(model, evo_net, t_pretrain_out=4, optimizer=None, optim_evo=None, lr_scheduler=None, loss_scaler=None, path=None, only_model=False):
    """
    加载模型
    """
    ...
    if path.exists():
        ...
        if only_model:
            ...
            evo_net.load_pretrained_weights(new_evo_state_dict, old_n_classes=t_pretrain_out)
        else:
            state_dict = ckpt['model']
            # 移除 'module.' 前缀（如果存在）
            new_state_dict = {}
...
def valid_one_epoch(epoch, model, evo_net, data_args, dataloaders, surface_mask, channel_weight, var1, var2, device="cuda:0"):
    ...
    for step, batch in enumerate(pbar):
        ...
        motion_ = motion.reshape(B, C, -1, H, W).reshape(B, C, data_args['t_finetune_out'], 2, H, W).permute(0,2,3,1,4,5)
        intensity_ = intensity.reshape(B, C, -1, H, W).reshape(B, C, data_args['t_finetune_out'], 1, H, W).permute(0,2,3,1,4,5) # b, t, 1, c, h, w
        ...
        # 多步演化, 每帧截断梯度
        for i in range(data_args['t_finetune_out']):
        ...
        evo_air = evo_result_detach[:,:,:-5,:,:].view(B, data_args['t_finetune_out'], 13, 5, H, W).permute(0,1,3,2,4,5)#batch size, t, variables, levels, lat, lon
        evo_surface = evo_result_detach[:,:,-5:,:,:] #batch size, t, variables, lat, lon

        pred_air, pred_surface = pangu_autoregressive_rollout(model, x_air, x_surface, surface_mask, evo_air, evo_surface, data_args['t_finetune_out'])
        # 计算加权MAE损失
...
def train(local_rank, model_args, data_args, proj):
    ...
    try:
        ...
        evo_net = Evolution_Network(n_channels=1, n_classes=data_args['t_finetune_out']).cuda()

        optimizer = torch.optim.AdamW(model.parameters(), lr=model_args.lr, betas=(0.9, 0.95),
                                    weight_decay=model_args.weight_decay)
        optim_evo = torch.optim.Adam(evo_net.parameters(), lr=1e-4, betas=(0.5,0.999))
        lr_scheduler, _ = create_scheduler(model_args, optimizer)

        start_epoch, start_step, min_loss = load_model(model, evo_net, t_pretrain_out=data_args['t_pretrain_out'], optimizer=optimizer, optim_evo=optim_evo, lr_scheduler=lr_scheduler, path=latest_model_path)
        if torch.cuda.device_count() > 1:
            model = DDP(model, device_ids=[gpu])

        # Dataloader
        train_dataset = WeatherPanguData(train_times, data_args['npy_path'], data_args['tp6hr_path'], input_window_size=data_args['t_in'], output_window_size=data_args['t_finetune_out'])
        ...
        valid_dataset = WeatherPanguData(valid_times, data_args['npy_path'], data_args['tp6hr_path'], input_window_size=data_args['t_in'], output_window_size=data_args['t_finetune_out'])
        ...
        for epoch in range(model_args.finetune_epochs):
            ...
            if gpu == 0:
                ...
                save_model(model, evo_net, epoch=epoch + 1, min_loss=min_loss, optimizer=optimizer, optim_evo=optim_evo, lr_scheduler=lr_scheduler, path=finetune_model_path)
                ...
                if val_score < best_score:
                    ...
                    min_loss = val_loss
                    save_model(model, evo_net, epoch=epoch, min_loss=min_loss, path=data_args['root_path'] / f"output/{proj}/ckpts/finetune_epoch{epoch + 1}_{val_score:.6f}_best.pt", only_model=True)
            dist.barrier()
    finally:
...
if __name__ == '__main__':
    ...
    proj='pangu_motion_0927_finetune'
    data_args = get_pangu_data_args('pangu_motion_0927')
    index = 1
    proj_base = 'pangu_motion_0927_finetune'
